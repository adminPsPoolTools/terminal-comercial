...
<div id="ct-visitas">
  <div class="flex flex-wrap items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white">
    ...
    <a href="{{ route('visitas.crear', ['cliente' => $codigo ?? '']) }}" class="btn btn-sm btn-primary">+ Nueva
      visita</a>
      ...
      <tbody>
        ...
        <tr>
          <td class="font-mono text-xs font-semibold">
            <a href="{{ route('visitas.detalle', $row->CODIGO ?? 0) }}" class="text-blue-600 hover:underline">{{
              $row->CODIGO ?? '—' }}</a>
          </td>
          ...
          <td class="text-xs text-slate-600 max-w-xs truncate">{{ $row->COMENTARIO ?? $row->COMENTARIO_MOTIVO ?? '—' }}
          </td>
        </tr>
        @endforeach
      </tbody>
...
<script>
  window.initCrmTable && window.initCrmTable('ct-visitas');
</script>
@endif