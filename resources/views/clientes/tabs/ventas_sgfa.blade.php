...
<div id="ct-ventas">
  <div class="flex flex-wrap items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white">
    <div class="relative">
      ...
      <input type="text" class="tab-search form-input pl-8 py-1.5 text-xs w-52" placeholder="Buscar ventas...">
    </div>
  ...
  <div class="table-wrapper">
    <table class="crm-table">
      <thead>
        <tr>
          <th class="srt">Sección <span class="sa text-slate-300">↕</span></th>
          <th class="srt">Grupo <span class="sa text-slate-300">↕</span></th>
          <th class="srt">Familia <span class="sa text-slate-300">↕</span></th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="font-mono text-xs font-semibold">
            <a href="{{ route('articulos.index') }}" class="text-blue-600 hover:underline" title="Ver artículo {{ $row->CODIGO ?? '' }}">
              {{ $row->CODIGO ?? '—' }}
            </a>
          </td>
...
<script>window.initCrmTable && window.initCrmTable('ct-ventas');</script>
@endif
