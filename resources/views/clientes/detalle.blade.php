...
<div class="space-y-5 max-w-5xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start gap-4">
      ...
      <div class="flex-1 min-w-0">
        ...
        <p class="text-slate-400 text-sm mt-0.5">
          Código: <span class="font-mono font-semibold text-slate-600">{{ $cliente->CODIGO ?? '—' }}</span>
          @if($cliente->DESCRIPCIONCATEGORIA ?? false)
            · <span class="badge badge-blue">{{ $cliente->DESCRIPCIONCATEGORIA }}</span>
          @endif
        </p>
        @if(!empty($cliente->COMENTARIO_ALERTA))
          <div class="mt-2 flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
            ...
            <p class="text-red-700 text-sm font-medium">{{ $cliente->COMENTARIO_ALERTA }}</p>
          </div>
        @endif
      </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div class="crm-card p-5">
      <h3 class="font-head font-semibold text-sm text-slate-800 mb-4 pb-3 border-b border-slate-100 uppercase tracking-wide">
        Datos de contacto
      </h3>
      <dl class="space-y-3 text-sm">
        @foreach([
          ['Dirección', ($cliente->DIRECCION ?? '').' '.($cliente->NUMERO ?? '')],
          ['CP / Población', ($cliente->CP ?? '').' '.($cliente->DESCRIPCION_POBLACION ?? '')],
          ['Provincia', $cliente->DESCRIPCION_PROVINCIA ?? ''],
          ['País', $cliente->DESCRIPCION_PAIS ?? ''],
    ...
    <div class="crm-card p-5">
      ...
      <dl class="space-y-3 text-sm">
        @foreach([
          ['Vendedor', $cliente->NOMBRE_VENDEDOR ?? $cliente->VENDEDOR ?? ''],
          ['Categoría', $cliente->DESCRIPCIONCATEGORIA ?? ''],
          ['Grupo', $cliente->DESCRIPCION_GRUPO ?? ''],
          ['Riesgo', $cliente->RIESGO ?? ''],
          ['F. alta', $cliente->FECHA_ALTA ?? ''],
          ['Último contacto', $cliente->ULTIMA_VISITA ?? ''],
          ['NIF / CIF', $cliente->NIF ?? ''],
        ] as [$label, $value])
          @if($value)
          <div class="flex gap-3">
  ...
  {{-- Tabs: sub-secciones del cliente --}}
...
@push('scripts')
<script>
var clienteCodigo = '{{ $cliente->CODIGO ?? "" }}';
var comercial = {{ session('comercial_id') }};
...
function switchTab(btn) {
  ...
  var urls = {
    'tab-agenda':     '/agenda/list?codigo_cliente='+clienteCodigo+'&comercial='+comercial,
    'tab-presup':     '/presupuestos/list?cliente='+clienteCodigo,
    'tab-pedidos':    '/pedidos/list?cliente='+clienteCodigo,
    'tab-expediente': '/expedientes/list?cliente_asign='+clienteCodigo,
    'tab-gastos':     '/gastos/list?cliente='+clienteCodigo,
    'tab-contactos':  '/clientes/'+clienteCodigo+'/contactos',
    'tab-direcciones':'/clientes/'+clienteCodigo+'/direcciones',
  };
  ...
  $.get(urls[tab], function(html) { $('#tab-content').html(html); })
   .fail(function() { $('#tab-content').html('<div class="empty-state">No disponible.</div>'); });
}

// Cargar primera pestaña al entrar
$(function() {
